 ...
 *
 * Starter configuration for a single-core Cortex-M target. Adjust to match
 * your MCU's clock and memory budget.
 */
...
/* Headroom for a few tasks, queues and timers without needing tuning on a
 * first build. Reduce it if the target is tight on RAM - the linker will fail
 * with a section-overflow if this does not fit. */
