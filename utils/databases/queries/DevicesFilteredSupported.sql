-- Filtered MCU list for device-first setup flow, gated to supported (released) devices
-- Parameters:
...
--   %20 = supported-devices filter active (1 = apply, 0 = skip)
--   %21 = supported device uid IN clause values, SQL-quoted comma-separated ('') when inactive)
...
WHERE
    ...
    -- Supported-devices filter: %20=0 skips; %21 always has at least '' to keep IN() syntactically valid
    AND (%20 = 0 OR Devices.uid IN (%21))
ORDER BY
    Devices.uid;
