-- Devices query (pure MCU list for devices-first setup flow), gated to supported (released) devices
-- Parameters:
--   %1  = unused (kept for positional parity with DevicesConfig.sql)
--   %2  = search text
--   %3  = supported-devices filter active (1 = apply, 0 = skip)
--   %4  = supported device uid IN clause values, SQL-quoted comma-separated ('') when inactive)
...
WHERE
    ...
    -- Supported-devices filter: %3=0 skips; %4 always has at least '' to keep IN() syntactically valid
    AND (%3 = 0 OR Devices.uid IN (%4))
ORDER BY
    Devices.uid;
